...
const register = async (server) => {
  ...
  server.slackCommand.register('status', (slackPayload, match) => {
    return getStatuses();
  }, 'list last known status for each url');
  // // triggers a check of all urls
  server.slackCommand.register('check', async (slackPayload, match) => {
    await server.inject({
      method: 'GET',
      url: '/api/runall',
    });
  ...
  server.slackCommand.register('certs', async (slackPayload, match) => {
    const response = await server.inject({
      method: 'GET',
      url: '/api/cert',
    });
  ...
  server.slackCommand.register('(.*)', async (slackPayload, match) => {

...
const register = async (server) => {
  ...
  server.slackCommands.register('status', (slackPayload, match) => {
    return getStatuses();
  }, 'list last known status for each url');
  // // triggers a check of all urls
  server.slackCommands.register('check', async (slackPayload, match) => {
    await server.inject({
      method: 'GET',
      url: '/api/runall',
    });
  ...
  server.slackCommands.register('certs', async (slackPayload, match) => {
    const response = await server.inject({
      method: 'GET',
      url: '/api/cert',
    });
  ...
  server.slackCommands.register('(.*)', async (slackPayload, match) => {
